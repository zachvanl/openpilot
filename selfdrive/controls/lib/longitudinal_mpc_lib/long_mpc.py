...
TFOLLOW_MIN_S = 0.80   # ISO 15622 style minimum time headway
TFOLLOW_MAX_S = 2.20

def get_T_FOLLOW(personality=log.LongitudinalPersonality.standard,
                 overrides: tuple[float, float, float] | None = None) -> float:
  """Time headway (s) for each personality. Optional overrides = (aggressive, standard, relaxed)."""
  if overrides is not None:
    t_agg, t_std, t_rel = overrides
    t_agg = float(np.clip(t_agg, TFOLLOW_MIN_S, TFOLLOW_MAX_S))
    t_std = float(np.clip(t_std, TFOLLOW_MIN_S, TFOLLOW_MAX_S))
    t_rel = float(np.clip(t_rel, TFOLLOW_MIN_S, TFOLLOW_MAX_S))
  else:
    t_agg, t_std, t_rel = 1.25, 1.45, 1.75
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, personality=log.LongitudinalPersonality.standard,
             ...
             t_follow_overrides: tuple[float, float, float] | None = None):
    t_follow = get_T_FOLLOW(personality, t_follow_overrides)
