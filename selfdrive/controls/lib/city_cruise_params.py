...
class CityCruiseParams:
  ...
  def from_params(cls, params: Params | None = None) -> "CityCruiseParams":
    ...
    return cls.from_values(
      ...
      approach_min_ft=float(p.get("CityApproachMinDistFt")),
      approach_max_ft=float(p.get("CityApproachMaxDistFt")),
      approach_min_closing_mph=float(p.get("CityApproachMinClosingMph")),
      approach_far_buffer_mph=float(p.get("CityApproachFarBufferMph")),
      established_coast_enabled=p.get_bool("CityEstablishedFollowCoast"),
      established_follow_s=float(p.get("CityEstablishedFollowTimeS")),
      late_brake_assist_enabled=p.get_bool("CityLateBrakeAssist"),
      coast_max_lead_alead=float(p.get("CityCoastMaxLeadAccel")),
      hard_brake_lead_alead=float(p.get("CityHardBrakeLeadAccel")),
    )
